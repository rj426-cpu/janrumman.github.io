...
#------------------------------
    ...
    class LATCH:
        ...
        error_message = "Signal '{name}' (driven in if-stmt) lacks a complete (every bit is assigned) top-level default in always_comb. At the very top of the always_comb block please assign a default value."

    class ASSIGNORDER:
        name = "ASSIGNORDER"
        ID = "R102"
        description = "Non-conditional assignments must appear at the top of an always_comb block, before any conditionals."
        error_message = "Non-conditional assignment found after a conditional statement. Move all default assignments to the top of the always block."
...
#------------------------------
    ...
    class CASEDEFAULT:
        ...
        error_message = "Case statement lacks a 'default' case. At the very end of the case statement please add: `default :`"

    class XASSIGN:
        name = "XASSIGN"
        ID = "R202"
        description = "To prevent X-optimism, signals driven in a case statement should be assigned 'x' in the default case."
        error_message = "Signal '{name}' is not assigned 'x' in the default case. In the default case please include {name} = 'x;`"
    ...
    class XPROP:
        ...
        error_message = "Signal '{name}' conditionally assigned in {type} in always_comb (always_ff) does not appear as the first argument of an ECE2300_XPROP (ECE2300_SEQ_XPROP) macro call found anywhere in this file. Please write something like `ECE2300_XPROP({name}, expression); in an always_comb block or `ECE2300_SEQ_XPROP({name}, expression); in an always_ff block."
...
#------------------------------
    ...
    class COMPLEXRHS:
        ...
        error_message = "RHS of 'assign' {detail_msg}. In gate-level/structural modules, RHS must be an identifier, identifier[msb:lsb], or a simple literal."
    ...
    class LOGICINPORT:
        name = "LOGICINPORT"
        ID = "R408"
        description = "Gate or Module instances cannot contain logical operations (like ~, &, ^) inside the port list."
        error_message = "Port connection contains a logical operation ({op_type}). You must use a wire/net."
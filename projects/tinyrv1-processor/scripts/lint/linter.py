    ...
    def _is_simple_netlist_target(self, target_node):
        ...
        if isinstance(target_node, Pointer):      
            if isinstance(target_node.var, Identifier) and isinstance(target_node.ptr, IntConst):
                return True
        if isinstance(target_node, Concat):
            return True
        return False
        ...
        if lhs_node and rhs_node:
            ...
            if is_lhs_simple_target:
                if not (is_rhs_simple_target or is_rhs_simple_literal):
                    ...
                    if isinstance(rhs_node, (Operator, UnaryOperator, Repeat, Pointer, FunctionCall, SystemCall)):
        ...
        for instance in node.instances:
            if instance.portlist:
                for port in instance.portlist:
                    if isinstance(port, WildcardPortArg):
                        continue
                    # port.argname is the expression connected to the port
                    # check if it is a UnaryOperator (e.g. ~sig) or Operator (e.g. a ^ b)
                    if isinstance(port.argname, (UnaryOperator, Operator)):
                        op_name = type(port.argname).__name__ # Gets 'Unot', 'Xor', 'And', etc.
                        self._add_violation(Rules.LOGICINPORT, node, op_type=op_name)
                    
                    # if isinstance(port.argname, Concat):
        ...
        try:
            ...
            if linter.violations:
                ...
                for mod_name, rule_id, msg, lineno in sorted_violations:
                    ...
                    header = f"    {module_prefix}: [{COLOR_RED}{rule_id}{STYLE_RESET}]"
                    message_body = f"       - {msg}"
                    print(header)
                    print(message_body)
            ...
            COLOR_RED = "\033[91m"
            COLOR_RESET = "\033[0m"
            print(f"{COLOR_RED}You have a syntax error that is causing our linter to be unable to parse your file. This could be things like unnecessary semicolons, not putting xprop in the correct place, forgetting semicolons etc. etc. Please try to fix the syntax error. If you believe you do not have syntax errors please find a TA or the professor.{COLOR_RESET}")            # error_message = f"Error processing file {f_path}: {e}"

            import traceback
            traceback.print_exc()
            total_violations_across_files +=1
